	...
	class WC_Admin_Dashboard {
		...
		public function init() {
			if ( current_user_can( 'publish_shop_orders' ) && post_type_supports( 'product', 'comments' ) ) {
				wp_add_dashboard_widget( 'woocommerce_dashboard_recent_reviews', __( 'ClassicCommerce recent reviews', 'woocommerce' ), array( $this, 'recent_reviews' ) );
			}
			wp_add_dashboard_widget( 'woocommerce_dashboard_status', __( 'ClassicCommerce status', 'woocommerce' ), array( $this, 'status_widget' ) );
		...
		public function register_network_order_widget() {
			wp_add_dashboard_widget( 'woocommerce_network_orders', __( 'ClassicCommerce network orders', 'woocommerce' ), array( $this, 'network_orders' ) );
		}
