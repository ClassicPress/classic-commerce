...
class WC_REST_Shipping_Zone_Methods_V2_Controller extends WC_REST_Shipping_Zones_Controller_Base {
	...
	public function register_routes() {
		register_rest_route(
			$this->namespace, '/' . $this->rest_base . '/(?P<zone_id>[\d]+)/methods', array(
				'args'   => array(
					'zone_id' => array(
						'description' => __( 'Unique ID for the zone.', 'classic-commerce' ),
						'type'        => 'integer',
					),
				),
				array(
		...
		register_rest_route(
			$this->namespace, '/' . $this->rest_base . '/(?P<zone_id>[\d]+)/methods/(?P<instance_id>[\d]+)', array(
				'args'   => array(
					...
					'instance_id' => array(
						'description' => __( 'Unique ID for the instance.', 'classic-commerce' ),
						'type'        => 'integer',
					),
				),
				array(
	...
	public function get_item( $request ) {
		...
		if ( false === $method ) {
			return new WP_Error( 'woocommerce_rest_shipping_zone_method_invalid', __( 'Resource does not exist.', 'classic-commerce' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( false === $method ) {
			return new WP_Error( 'woocommerce_rest_shipping_zone_not_created', __( 'Resource cannot be created.', 'classic-commerce' ), array( 'status' => 500 ) );
		}
	...
	public function delete_item( $request ) {
		...
		if ( $force ) {
			...
			return new WP_Error( 'rest_trash_not_supported', __( 'Shipping methods do not support trashing.', 'classic-commerce' ), array( 'status' => 501 ) );
		}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				'id'                 => array(
					'description' => __( 'Shipping method instance ID.', 'classic-commerce' ),
					'type'        => 'integer',
				...
				'title'              => array(
					'description' => __( 'Shipping method customer facing title.', 'classic-commerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'order'              => array(
					'description' => __( 'Shipping method sort order.', 'classic-commerce' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
				),
				'enabled'            => array(
					'description' => __( 'Shipping method enabled status.', 'classic-commerce' ),
					'type'        => 'boolean',
					'context'     => array( 'view', 'edit' ),
				),
				'method_id'          => array(
				...
				'settings'           => array(
					'description' => __( 'Shipping method settings.', 'classic-commerce' ),
					'type'        => 'object',
					'context'     => array( 'view', 'edit' ),
					'properties'  => array(
						'id'          => array(
